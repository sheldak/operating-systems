...
int main(int argc, char **argv) {
    ...
    if(domain == AF_UNIX) {
        ...
        // binding name
        if(bind(socket_fd, (struct sockaddr*) &unix_address, sizeof(sa_family_t)) == -1)
        ...
        if(inet_pton(AF_INET, argv[3], &bind_address.sin_addr) < 0) perror("Invalid IP address");

        bind_address.sin_port = 0;

        if(bind(socket_fd, (struct sockaddr*) &bind_address, sizeof(bind_address)) == -1)
