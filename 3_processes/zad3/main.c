...
void setResLimits(int timeLim, int memoryLim){
    memoryLim *= 1048576;

    struct rlimit timeRLimit;
    struct rlimit memRLimit;

    timeRLimit.rlim_max = (rlim_t) timeLim;
    timeRLimit.rlim_cur = (rlim_t) timeLim;
    if(setrlimit(RLIMIT_CPU, &timeRLimit) != 0)
        perror("cannot set time limits");

    memRLimit.rlim_max = (rlim_t) memoryLim;
    memRLimit.rlim_cur = (rlim_t) memoryLim;
    if(setrlimit(RLIMIT_AS, &memRLimit) != 0)
        perror("cannot set memory limits");
}
...
void writeUsageReport(struct rusage usageStart, struct rusage usageEnd) {
    long int userUsage = labs(getUsageTime(&usageEnd.ru_utime) - getUsageTime(&usageStart.ru_stime));
    long int systemUsage = labs(getUsageTime(&usageEnd.ru_stime) - getUsageTime(&usageStart.ru_stime));

    printf("System time usage: %lf\n", (double) systemUsage / 1000000);
...
int main(int argc, char **argv) {
    ...
    // fifth argument, limit on time usage
    rest = "";
    int timeLimit = (int) strtol(argv[5], &rest ,10);

    if(strcmp(rest, "") != 0)
        perror("invalid third argument");

    // sixth argument, limit on memory usage
    rest = "";
    int memLimit = (int) strtol(argv[6], &rest ,10);
    ...
    int *pids = calloc(processesNum, sizeof(int));
    ...
    if(myPID == 0) {
        setResLimits(timeLimit, memLimit);
        ...
        for(int i=0; i<processesNum; i++) {
            ...
            writeUsageReport(usageStart, usageEnd);
        }
